...
class Dependency:
    def __init__(self, id, form, tag, head, rel, charid, senid):
        self.id = id
        self.org_form = form
    ...
    def __str__(self):
        values = [str(self.id), self.org_form, "_", self.tag, "_", "_", str(self.head), self.rel, "_", "_", str(self.charid),str(self.senid)]
        return '\t'.join(values)
...
def readDepTree(file, vocab=None):
    ...
    if vocab is None: sentence = []
        ...
        sentence = [Dependency(0, vocab._root_form, vocab._root, 0, vocab._root, -1,senid)]
        #senid+=1
    for line in file:
        ...
        if not tok or line.strip() == '' or line.strip().startswith('#'):
            ...
            if tok[6] == '_': tok[6] = '-1'
            try:
                sentence.append(Dependency(int(tok[0]), tok[1], tok[3], int(tok[4]), tok[5], charid, senid))
                charid+=1
            except Exception:
                pass
        else:
