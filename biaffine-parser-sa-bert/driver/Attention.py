...
class MyAttentionEncoder(nn.Module):
    def __init__(self,vocab,config,pretrained_embedding,
        ...
        self.n_max_seq=n_max_seq+1
        self.d_model=d_model
        self.config=config
        self.position_enc=nn.Embedding(self.n_max_seq,config.word_dims,padding_idx=0)
        self.position_enc.weight.data=position_encoding_init(self.n_max_seq,config.word_dims)
        ...
        #self.bias = nn.Parameter(torch.ones(100), requires_grad=True)
        ...
        self.linear=nn.Linear(768,100,bias=False)
    ...
    def forward(self,word_seq,extwords,tags,masks,pos_seq,sens,elmosens,berts,elmofile,return_attn=False):
        ...
        if use_bert:
            ...
            char_idxs_encoder = np.zeros((len(elmosens), max_sen_len, 768) ,dtype=np.float32)
            for i in range(len(elmosens)):
                length=int(elmosens[i][0])-1
                bert=int(berts[i])
                #print('senid',bert)
                #print('lens:',len(elmofile))
                #print("sen length",length)
                #print("bert size",elmofile[bert].size())
                for j in range(length):
                    char_idxs_encoder[i][j+1]=elmofile[bert][j][3]#the fourth layer
                    '''
                    for k in range(768):
                        char_idxs_encoder[i][j+1][k]=float(elmofile[bert][j][0][k])
                    '''
            elmo_emb=Variable(torch.from_numpy(char_idxs_encoder).type(torch.cuda.FloatTensor),requires_grad=False)
            extenc_input = self.linear(elmo_emb)
        ...
        position_input = self.position_enc(pos_seq)
        if self.training:
            enc_input, tag_input = drop_input_independent(enc_input, tag_input, self.config.dropout_emb)
            #position_input=drop_sequence_sharedmask(position_input,self.config.dropout_emb)
        #nc_input = torch.cat((enc_input,tag_input),dim=2)
        enc_input = enc_input+tag_input
        #enc_input = enc_input+self.bias
        #enc_input += self.position_enc(pos_seq)#mb_size*n_max_seq*d_word_vec
        enc_input =torch.cat((enc_input,position_input),dim=2)
        if self.training:
            enc_input = drop_sequence_sharedmask(enc_input,self.config.dropout_emb) 
        if return_attn:
            enc_slf_attns=[]
