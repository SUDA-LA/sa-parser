...
class BiaffineParser(object):
    ...
    def forward(self, words, extwords, tags, masks, positions,sens,elmosens,berts,elmofile):
        if self.use_cuda:
            ...
            #sens =sens.cuda(self.device)

        arc_logits, rel_logits = self.model.forward(words, extwords, tags, masks, positions,sens,elmosens,berts,elmofile)
        # cache
    ...
    def compute_accuracy(self, true_arcs, true_rels):
        ...
        arc_correct=int(arc_correct)
        label_correct=int(label_correct)
        total_arcs=int(total_arcs)

        return arc_correct, label_correct, total_arcs

    def parse(self, words, extwords, tags, lengths, masks, positions,sens,elmosens,berts,elmofile):
        if words is not None:
            self.forward(words, extwords, tags, masks, positions,sens,elmosens,berts,elmofile)
        ROOT = self.root
